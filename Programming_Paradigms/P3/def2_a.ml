...
let absf n = if n<0. then n -. 2. *. n else n;;
...
let is_a_letter c = if Char.code c >= 65 && Char.code c <= 122 then true else false;;

(*let string_of_bool = function b -> if b = false then "falso" else "verdadero";;*)
let string_of_bool b = if b = false then "falso" else "verdadero";;
